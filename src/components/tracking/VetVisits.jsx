...
export default function VetVisits({ petId }) {
  ...
  const inputClass = "border rounded-xl px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-amber-400"
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-gray-800">🏥 Vet Visits</h3>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-amber-50 rounded-xl p-4 mb-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Next visit</label>
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" disabled={adding}
              className="flex-1 bg-amber-500 text-white rounded-xl py-1.5 text-sm disabled:opacity-50">
              {adding ? 'Saving...' : 'Save'}
            </button>
          </div>
        </form>
      ...
      {loading ? <p className="text-sm text-gray-400">Loading...</p> : visits.length === 0 ? (
        <p className="text-sm text-gray-400 text-center py-6">No vet visits logged yet</p>
      ) : (
        <div className="flex flex-col gap-2">
          {visits.map(visit => (
            <div key={visit.id} className="bg-gray-50 rounded-xl px-4 py-3">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium text-gray-800">{visit.reason}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {new Date(visit.visit_date).toLocaleDateString([], { dateStyle: 'medium' })}
                    {visit.vet_name && ` · ${visit.vet_name}`}
                  </p>
                  {visit.next_visit && (
                    <p className={`text-xs mt-1 font-medium ${isUpcoming(visit.next_visit) ? 'text-amber-500' : 'text-gray-400'}`}>
                ...
                <button onClick={() => handleDelete(visit.id)} className="text-xs text-red-300 hover:text-red-500 ml-4">✕</button>
              </div>
            </div>
          ))}
        </div>
