...
export default function Chat({ pet, recentMeals, recentWeight, recentVetVisits }) {
  ...
  // Build pet context string from real data
  const petContext = `
Pet Name: ${pet.name}
Species: ${pet.species}
  ...
  return (
    <div className="flex flex-col h-[600px]">
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          <h3 className="font-semibold text-gray-800">🤖 Ask PawPal AI</h3>
          <p className="text-xs text-gray-400">Personalized advice for {pet.name}</p>
        </div>
        {messages.length > 0 && (
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            ...
            <p className="text-sm text-gray-400 text-center">
              Ask me anything about {pet.name}!<br />I already know their profile and history.
            </p>
            {/* Suggestion chips */}
            <div className="flex flex-col gap-2 w-full mt-2">
              {SUGGESTIONS.map(s => (
                <button key={s} onClick={() => { sendMessage(s) }}
                  className="text-left text-xs bg-violet-50 hover:bg-violet-100 text-violet-700 px-4 py-2.5 rounded-xl transition-colors">
                  {s}
                </button>
              ))}
            </div>
          ...
          messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'assistant' && (
                <div className="w-7 h-7 rounded-full bg-violet-100 flex items-center justify-center text-sm mr-2 flex-shrink-0 mt-1">
                  🐾
                </div>
              )}
